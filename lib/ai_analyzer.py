...
class OllamaAnalyzer:
    ...
    def analyze_script(self, script_path: str) -> Optional[Dict]:
        ...
        if len(script_content) > 4000:
            script_content = script_content[:4000] + "\n... (truncated)"
        
        # Create analysis prompt
        prompt = f"""Analyze this bash script and respond with ONLY valid JSON (no markdown, no explanations).
...
Categories: install, tools, exercises, uninstall
Purpose: install_software, system_tool, learning, cleanup, utility
Safety: safe, caution, requires_review
